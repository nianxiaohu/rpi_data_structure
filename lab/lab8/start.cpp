...
int totalPath(int, int);
int totalPathWithBlock(int,int,vector< vector<bool> >&);
int main(int argc, char* argv[]) {
  ...
  // check point 1
  int total = 0;
  total  = totalPath(start_x,start_y);
  cout << "The number of path from point ("<< start_x<<","<< start_y <<") to (0,0) without blocking is " << total << endl;
  // check point 2
  total = totalPathWithBlock(start_x,start_y, blocked_grid);
  cout << "The number of path from point ("<< start_x<<","<< start_y <<") to (0,0) with blocking is " << total << endl;


  return 0;
}
// the totalPath from any point at the x or y axis is 1
// the totalPath from other points(x,y) is the sum of totalPath(x-1,y) + totalPath(x,y-1).
int totalPath( int x, int y) {
  if ( x == 0 || y == 0 )
    return 1;
  else
    return totalPath(x-1,y) + totalPath(x,y-1);
}

int totalPathWithBlock( int x, int y, vector< vector<bool> >& board) {
  // if the point is one of the blocking point then return 0
  if ( board[x][y] ) 
    return 0;
  else {
    if ( x==0 && y==0) 
      ...
      return totalPathWithBlock( x,y-1,board);
    else if ( y==0 )
      return totalPathWithBlock( x-1,y,board);
    else 
      return totalPathWithBlock( x,y-1,board) + totalPathWithBlock(x-1,y,board);
  }
}
