...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    // if the pointer is NULL, return else create a new node and recursively call left and right tree of copy_tree
    if ( !old_root ) 
      return NULL;
  ...
  */
  // non recursive version of find
  iterator find(const T& key_value, TreeNode<T>* p) {
    if (!p) return iterator(NULL);
    while ( key_value != p->value ) {
      if ( key_value < p->value && p->left != NULL ) { p = p->left; }
      else if ( key_value > p->value && p->right != NULL ) { p = p->right; }
